...
def _trace_with_cost(cost: float, hours_ago: float) -> dict:
    return {
        ...
        "name": "rca",
        "timestamp": (datetime.now(tz=UTC) - timedelta(hours=hours_ago)).isoformat(),
...
def test_cost_spike_fires_when_24h_doubles_baseline(session: Session):
    ...
    fake_lf.search_traces.return_value = [
        _trace_with_cost(0.10, 30),
        _trace_with_cost(0.12, 50),
        _trace_with_cost(0.08, 100),
    ] + [
        _trace_with_cost(0.50, 5),
        _trace_with_cost(0.60, 12),
        _trace_with_cost(0.40, 20),
    ]
...
def test_cost_spike_no_signal_when_stable(session: Session):
    ...
    fake_lf.search_traces.return_value = [
        _trace_with_cost(0.10, 30),
        _trace_with_cost(0.12, 50),
        _trace_with_cost(0.10, 100),
    ] + [
        _trace_with_cost(0.11, 5),
        _trace_with_cost(0.10, 12),
        _trace_with_cost(0.13, 20),
    ]
    signal = detect_cost_spike_for_agent(
        agent=agent,
        session=session,
    ...
    fake_lf.search_traces.return_value = [
        {
            "id": "t1",
            "name": "rca",
            "timestamp": (datetime.now(tz=UTC) - timedelta(hours=5)).isoformat(),
            "metadata": {},
            "scores": [],
        }
    ]
